...
// CrewSplit brand mark. Uses `currentColor` so it adapts to the surrounding
// text color — pair it with `text-foreground` to get a dark mark on light
// backgrounds and a white mark on dark backgrounds automatically.
const LOGO_PATH =
...
// Selectable brand-logo colors (stored in the `app_icon` setting).
// `auto` inherits the text color so the mark stays dark on light backgrounds
// and white on dark — the rest are fixed accent colors from the theme.
export const LOGO_COLORS = [
  { token: 'auto', class: 'text-foreground' },
  { token: 'primary', class: 'text-primary' },
  { token: 'info', class: 'text-info' },
  { token: 'success', class: 'text-success' },
  { token: 'warning', class: 'text-warning' },
  { token: 'danger', class: 'text-destructive' },
  { token: 'boat1', class: 'text-boat1' },
  { token: 'boat2', class: 'text-boat2' },
] as const;

/** Map a stored color token to its Tailwind text class (falls back to auto). */
export function logoColorClass(token?: string | null): string {
  return LOGO_COLORS.find(c => c.token === token)?.class ?? 'text-foreground';
}
